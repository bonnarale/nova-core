...
class APITracer:
    """Traces incoming requests through middleware, DI, endpoint execution, and response."""
    ...
    def start_trace(self, request_id: str = "", path: str = "", method: str = "GET") -> str:
                ...
                if s["trace_id"] == trace_id:
                    for sp in reversed(s["spans"]):
                        if sp["name"] == name and sp["finished_at"] == 0.0:
                            ...
                            sp["duration_ms"] = round((sp["finished_at"] - sp["started_at"]) * 1000, 2)
                            break
                    break
        ...
        with self._lock:
            for s in reversed(self._spans):
                if s["trace_id"] == trace_id:
                    ...
                    s["duration_ms"] = round((s["finished_at"] - s["started_at"]) * 1000, 2)
                    s["status"] = status
                    break
