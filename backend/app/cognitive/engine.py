...
class CognitiveEngine:
    ...
    async def _load_context(
        ...
        # Semantic memory: auto-index loaded profile / goals / tasks
        if self._semantic_memory is not None:
            try:
                await self._semantic_memory.auto_index(
                    profile=profile if user_id else None,
                    goals=ctx.goals,
                    tasks=all_tasks if user_id else None,
                )
            except Exception:
                logger.debug("Could not auto-index semantic memory")

            try:
                semantic_results = await self._semantic_memory.search(
                    raw_input, top_k=self._semantic_memory.config.top_k or 3
                )
                if semantic_results:
                    ctx.extra["semantic_memories"] = semantic_results
            except Exception:
        ...
        if self._long_term_memory is not None and raw_input.strip():
            try:
                ltm_results = await self._long_term_memory.search(
                    query=raw_input,
                    user_id=user_id,
                    limit=5,
                )
                if ltm_results.results:
                    ctx.extra["long_term_memories"] = [
                        m.to_dict() for m in ltm_results.results
                    ]
            except Exception:
                logger.debug("Could not search long-term memory")

        # Vector Memory: retrieve semantically similar vector memories
        if self._vector_memory_engine is not None and raw_input.strip():
            try:
                from app.vector_memory.schemas import VectorMemoryQuery
                vm_results = await self._vector_memory_engine.search(
                    VectorMemoryQuery(
                        query=raw_input,
                        user_id=user_id,
                        top_k=5,
                ...
                if vm_results.results:
                    ctx.extra["vector_memories"] = [
                        r.model_dump() for r in vm_results.results
                    ]
            except Exception:
                logger.debug("Could not search vector memory")
    ...
    async def _execute_create_plan(
        ...
        user_id = decision.payload.get("user_id", context.user_id)
        session_id = decision.payload.get("session_id", context.session_id)
        ...
        result = await self._planner.execute_plan(plan)
        return {"created_plan": result.to_dict(), "objective": objective}
    ...
    async def _execute_reason(
        ...
        return {"reasoning_decision": result.to_dict(), "query": query}
    ...
    async def _execute_workflow(
        ...
        # Try to find a workflow by name matching the task
        workflows = self._workflow_engine.list_workflows()
        matched = None
        for w in workflows:
            if w.name.lower() in task.lower() or task.lower() in w.name.lower():
                matched = w
                break

        if matched is None:
            return {"error": "No matching workflow found", "task": task}

        execution = await self._workflow_engine.create_execution(
            workflow_id=matched.id,
            input_data={"task": task, "user_input": task},
            user_id=user_id,
            session_id=session_id,
        )
        execution = await self._workflow_engine.start_execution(execution.id)
        return {"workflow_execution": execution.to_dict(), "workflow_name": matched.name}
    ...
    async def _execute_knowledge_graph(
        ...
        results = await self._knowledge_graph_engine.search(query=query, limit=10)
        return {
            "query": query,
    ...
    async def _execute_create_task(
        ...
        result = {"created_task": task, "assigned_agent": agent}
