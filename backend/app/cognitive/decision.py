...
class OpenCodeHandler(DecisionHandler):
    """Handler for SDD task intents — delegate to OpenCode subprocess."""

    @property
    def handled_intents(self) -> set[IntentType]:
        return {IntentType.SDD_TASK}

    async def decide(self, context: CognitiveContext) -> CognitiveDecision:
        return CognitiveDecision(
            action=DecisionAction.DELEGATE_TO_OPENCODE,
            handler_name="opencode",
            payload={
                "task": context.raw_input,
                "user_id": context.user_id,
            },
            confidence=0.9,
            reasoning="SDD task detected — delegating to OpenCode subprocess.",
        )
...
class MetaHandler(DecisionHandler):
    """Handler for self-improvement and meta-evolution intents."""

    @property
    def handled_intents(self) -> set[IntentType]:
        return {IntentType.META_IMPROVEMENT}

    async def decide(self, context: CognitiveContext) -> CognitiveDecision:
        return CognitiveDecision(
            action=DecisionAction.RUN_META_CYCLE,
            handler_name="meta",
            payload={
                "task": context.raw_input,
                "user_id": context.user_id,
            ...
            confidence=0.8,
            reasoning="Meta-improvement request detected — routing to evolution engine.",
        )
